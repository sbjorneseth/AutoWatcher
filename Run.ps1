    ...
    foreach ($Streamer in $Streamers) {
        ...
        if ($Response -match '"isLiveBroadcast":.*true') {
            ...
            while ($Streamer.WatchTimeMinutes -lt $Settings.WatchTimeGoalMinutes) {
                ...
                $Response = Invoke-RestMethod -Method Get -Uri "https://www.twitch.tv/$($Streamer.Name)"
                if ($Response -match '"isLiveBroadcast":.*true') {
                    $Streamer = Get-Content -Path "$PSScriptRoot\Streamers.json" | ConvertFrom-Json | Where-Object -FilterScript {$_.Name -eq $Streamer.Name}
                    ...
                    $Progress = Get-Content -Path "$PSScriptRoot\Streamers.json" | ConvertFrom-Json
                    # Update progress
                    $Progress | ForEach-Object {if ($_.Name -eq $Streamer.Name) {[pscustomobject]@{Name = $_.Name;WatchTimeMinutes = $Streamer.WatchTimeMinutes}}else{$_}} | ConvertTo-Json | Set-Content -Path "$PSScriptRoot\Streamers.json" -Force
                }
                else {
